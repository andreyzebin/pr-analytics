...
def make_db(tmp_path):
    ...
    conn = pa.open_db(db_path)
    pa.upsert_project(conn, "PROJ", "Project")
    pa.upsert_repo(conn, 1, "PROJ", "backend", "Backend")
    conn.commit()
    return db_path, conn
...
def insert_pr(conn, pr_id, state="MERGED", created=1_700_000_000_000,
    ...
    """closed default = created + 100 hours."""
    conn.execute(
...
def test_status_missing_db_exits_4(tmp_path):
    with pytest.raises(SystemExit) as exc:
        pa.cmd_status(args(db=str(tmp_path / "nope.db")), {})
...
def test_find_repos_returns_repo(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    insert_pr(conn, 1, reviewers=["ivan"])
    conn.commit()
    conn.close()
    pa.cmd_find_repos(args(db=db_path, reviewer="ivan"), {})
    out = capsys.readouterr().out
...
def test_find_repos_no_match_exits_4(tmp_path):
    db_path, conn = make_db(tmp_path)
    insert_pr(conn, 1, reviewers=["someone-else"])
    conn.commit()
    conn.close()
    with pytest.raises(SystemExit) as exc:
        pa.cmd_find_repos(args(db=db_path, reviewer="ivan"), {})
...
def test_find_repos_state_filter(tmp_path, capsys):
    ...
    conn.commit()
    conn.close()
    pa.cmd_find_repos(args(db=db_path, reviewer="ivan", state="MERGED"), {})
    out = capsys.readouterr().out
    # Should find exactly one repo (deduplicated)
    assert out.strip().count("PROJ/backend") == 1
...
def test_find_repos_writes_file(tmp_path):
    ...
    pa.cmd_find_repos(args(db=db_path, reviewer="ivan", output=out_file), {})
...
def test_sql_select(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    conn.close()
    pa.cmd_sql(args(db=db_path, query="SELECT 1+1 AS n", limit=100), {})
    out = capsys.readouterr().out
    assert "2" in out
...
def test_sql_rejects_insert(tmp_path):
    ...
    conn.close()
    with pytest.raises(SystemExit) as exc:
        pa.cmd_sql(args(db=db_path, query="INSERT INTO projects VALUES('x','y',NULL)", limit=0), {})
...
def test_sql_rejects_drop(tmp_path):
    ...
    conn.close()
    with pytest.raises(SystemExit) as exc:
        pa.cmd_sql(args(db=db_path, query="DROP TABLE projects", limit=0), {})
    assert exc.value.code == 5
...
def test_sql_with_cte(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    conn.close()
    pa.cmd_sql(args(db=db_path, query="WITH x AS (SELECT 42 AS v) SELECT v FROM x", limit=100), {})
    out = capsys.readouterr().out
    assert "42" in out
...
def test_sql_json_format(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    conn.close()
    pa.cmd_sql(args(db=db_path, query="SELECT key FROM projects", limit=100, format="json"), {})
    out = capsys.readouterr().out
    data = json.loads(out)
...
def test_sql_csv_format(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    conn.close()
    pa.cmd_sql(args(db=db_path, query="SELECT key FROM projects", limit=100, format="csv"), {})
    out = capsys.readouterr().out
    assert "key" in out
    assert "PROJ" in out
...
def test_sql_output_file(tmp_path):
    ...
    pa.cmd_sql(args(db=db_path, query="SELECT key FROM projects", limit=100,
...
def _seed_feedback(conn):
    insert_pr(conn, 10, state="MERGED")
    conn.commit()
    # Root comment by AI agent
    conn.execute(
        """INSERT INTO pr_comments
           (id, repo_id, pr_id, parent_id, author, text, created_date, updated_date,
            severity, state, file_path, line, line_type, file_type)
           VALUES(1001, 1, 10, NULL, 'ai-bot', 'NPE risk here', 1700000000000, 1700000000000,
                  'BLOCKER', 'OPEN', 'src/Foo.java', 42, 'ADDED', 'TO')"""
    )
    # Reaction: +1 from bob
    conn.execute(
        "INSERT INTO comment_reactions(comment_id, author, emoji) VALUES(1001, 'bob', '+1')"
    )
    # Reaction: -1 from carol
    conn.execute(
        "INSERT INTO comment_reactions(comment_id, author, emoji) VALUES(1001, 'carol', '-1')"
    )
    # Reply from bob (human)
    conn.execute(
        """INSERT INTO pr_comments
           (id, repo_id, pr_id, parent_id, author, text, created_date, updated_date,
            severity, state, file_path, line, line_type, file_type)
...
def test_review_feedback_basic(tmp_path, capsys):
    ...
    _seed_feedback(conn)
    conn.close()
    pa.cmd_review_feedback(
        args(db=db_path, author="ai-bot", min_reactions=0, format="table"), {}
    )
    out = capsys.readouterr().out
    assert "NPE risk here" in out
    assert "PROJ/backend" in out
...
def test_review_feedback_reaction_counts(tmp_path):
    ...
    # Capture via JSON format to inspect structured data
    import io as _io
    ...
    assert len(data) == 1
    row = data[0]
    assert row["reactions_positive"] == 1   # bob's +1
    assert row["reactions_negative"] == 1   # carol's -1
    assert row["replies_count"] == 1
    assert row["replies"][0]["author"] == "bob"


def test_review_feedback_min_reactions_filter(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    _seed_feedback(conn)
    # Add a second comment with no reactions
    conn.execute(
        """INSERT INTO pr_comments
           (id, repo_id, pr_id, parent_id, author, text, created_date, updated_date,
            severity, state, file_path, line, line_type, file_type)
    ...
    data = json.loads(Path(out_file).read_text())
    assert len(data) == 1
    assert data[0]["comment_id"] == 1001  # only the one with reactions


def test_review_feedback_no_data_exits_0(tmp_path, capsys):
    db_path, conn = make_db(tmp_path)
    conn.close()
    pa.cmd_review_feedback(args(db=db_path, author="nobody", min_reactions=0), {})
    out = capsys.readouterr().out
...
def test_date_to_ms_round_trip():
    ms = pa.date_to_ms("2026-01-15")
    assert pa.ms_to_date(ms) == "2026-01-15"


def test_date_to_ms_end_of_day():
    start = pa.date_to_ms("2026-01-15")
    end = pa.date_to_ms("2026-01-15", end_of_day=True)
    assert end > start
    # difference should be ~23h 59m 59s in ms
...
def test_format_output_csv():
    ...
    out = pa.format_output(data, ["a", "b"], "csv")
...
def test_format_output_json():
    data = [{"a": 1}]
    out = pa.format_output(data, ["a"], "json")
    assert json.loads(out) == [{"a": 1}]


def test_format_output_table_contains_data():
    data = [{"col": "value123"}]
    out = pa.format_output(data, ["col"], "table")
