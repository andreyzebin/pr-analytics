...
def cmd_cache(args: argparse.Namespace, cfg: dict) -> None:
    ...
    # label → (start_time, status_string)
    _active: dict[str, tuple[float, str]] = {}
    _done_times: list[float] = []   # elapsed per completed repo
    ...
    # Background heartbeat — prints in-progress repos every 8s
    ...
    def _heartbeat() -> None:
        while not _stop_hb.wait(timeout=8):
            ...
            if parts:
                elapsed_total = time.monotonic() - _global_start
                print(
                    f"  ⟳ [{_fmt_elapsed(elapsed_total)}]  " + "  |  ".join(parts),
                    flush=True,
                )
    ...
    def cache_repo(proj_key: str, repo_slug: str) -> int:
        ...
        try:
            ...
            prs = paginate(session, f"{pr_url}?state=ALL")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_cache.add_argument("--concurrency", type=int, default=None,
